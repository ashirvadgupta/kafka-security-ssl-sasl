...
--network kafka-cluster-network confluentinc/cp-kafka:5.5.1 kafka-configs --zookeeper zookeeper-1:22181 --alter --add-config \
...
-v ${KAFKA_SASL_SCRAM_SECRETS_DIR}/zookeeper_client_jaas.conf:/etc/kafka/secrets/zookeeper_client_jaas.conf  \
-e KAFKA_OPTS="-Djava.security.auth.login.config=/etc/kafka/secrets/zookeeper_client_jaas.conf" \
--network kafka-cluster-network confluentinc/cp-kafka:5.5.1 kafka-configs --zookeeper zookeeper-1:22181 \
